...
public class ShowLocationList extends Activity {
	...
	String store_UserID, store_CorpID, store_UserName, store_password,
			...
			show_check_form_locto, show_start_date, show_end_Date,
			show_WeekDate, show_BERStatus, var_LocationID, var_ExpenseTypeID,
			var_StartDate, var_EndDate, var_Rate;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * show_check_form=getIntent().getExtras().getString("LocationForm");
		 * show_check_form_locto
		 * =getIntent().getExtras().getString("LocationTo");
		 */
		...
		if (isInternetPresent) {
			...
			Toast.makeText(ShowLocationList.this,
		...
		findViewById(R.id.Back).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				finish();
				// Intent in_next=new
				// Intent(ShowLocationList.this,InputForm.class);
				...
				// /*bd.putString("LocName", show_Name);*/
	...
	private class verify_user_login_asynchronously extends
		...
		ProgressDialog dialog = new ProgressDialog(ShowLocationList.this);
		...
		protected void onPostExecute(Boolean is_authenticated) {
			if (ShowLocationList.this != null
					&& !ShowLocationList.this.isFinishing())
				dialog.dismiss();
			view_data_in_List();
		}
	}
		...
		try {
			...
			for (int i = 0; i < contacts_List.length(); i++) {
				...
				for (int j = 0; j < contacts_LocationRate.length(); j++) {
					...
					if (show.getString("ID").length() > 0) {
						var_LocationID = show.getString("ID");
					} else if (show.getString("ExpenseTypeID") != null
							&& show.getString("ExpenseTypeID").length() > 0) {
						var_ExpenseTypeID = show.getString("ExpenseTypeID");
					} else if (show.getString("StartDate").length() > 0) {
						var_StartDate = show.getString("StartDate");
					} else if (show.getString("EndDate").length() > 0) {
						var_EndDate = show.getString("EndDate");
					} else if (show.getString("Rate").length() > 0) {
						var_Rate = show.getString("Rate");
					}
